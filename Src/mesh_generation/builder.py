...
class MeshBuilder:
    ...
    def export_ply(
        ...
        """
        Export mesh to PLY (Stanford Polygon) format with optional per-vertex normals.

        PLY is a lossless, widely-supported point/mesh interchange format (read by
        MeshLab, CloudCompare, Open3D, Blender). We write it directly rather than
        through pygltflib so PLY export works even where the glTF dependency is absent.

        Coordinates are written in the same local, origin-rebased frame as the GLB
        export (no CRS re-projection here); the CRS/origin live in the group manifest.

        Args:
            output_path: Target path; the suffix is forced to ".ply".
            vertices: (N, 3) float array of vertex positions.
            faces: (M, 3) int array of triangle vertex indices (0-based).
            building_id: Identifier, written as a PLY comment for traceability.
            normals: Optional (N, 3) float array of per-vertex normals.
            binary: If True (default) write binary_little_endian, else ascii.
        """
        try:
            ...
            if has_normals:
                ...
                if normals.shape != vertices.shape:
                    logger.warning(
                        f"PLY export: normals shape {normals.shape} != vertices "
                        f"{vertices.shape}; writing without normals"
                    )
                    has_normals = False
            ...
            header_lines = [
                "ply",
                f"format {fmt}",
                f"comment Building {building_id}",
                "comment Generated by MeshBuilder (local origin-rebased frame)",
            ...
            if binary:
                # Interleave vertex xyz (+ optional normals) as a structured record.
                if has_normals:
                    vdata = np.empty((len(vertices), 6), dtype=np.float32)
                    vdata[:, 0:3] = vertices
                    vdata[:, 3:6] = normals
                else:
                    vdata = vertices
                # Face records: uchar count (always 3) + 3 uint32 indices, packed rows.
                counts = np.full((len(faces), 1), 3, dtype=np.uint8)
                face_bytes = b"".join(
                    counts[i].tobytes() + faces[i].tobytes() for i in range(len(faces))
                )
    ...
    def export_glb_suite(
        ...
        for lod_level in ["lod1", "lod2", "lod3"]:
            ...
            # Compute normals if requested (shared across all output formats)
            normals = None
